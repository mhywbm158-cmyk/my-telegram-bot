...
async def keep_alive_job(context: ContextTypes.DEFAULT_TYPE):
    try:
        # Ping بسيط يبقي اتصال البوت نشط
        await context.bot.get_me()

        # لمسة خفيفة لقاعدة البيانات حتى لا تعتبر خاملة
        conn = get_db()
...
async def main_menu_display(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    btns = [
        [InlineKeyboardButton(t(user.id, 'get_link'), callback_data='get_my_link')],
        [InlineKeyboardButton(t(user.id, 'send_msg'), callback_data='btn_send'),
         InlineKeyboardButton(t(user.id, 'donate_btn'), callback_data='btn_donate')],
...
async def admin_home(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = [
        ...
        [InlineKeyboardButton("📊 إحصائيات", callback_data='adm_stats'), InlineKeyboardButton("📡 إذاعة", callback_data='
